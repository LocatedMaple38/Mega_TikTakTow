void reset(){
  start = false;  
  exet = false;  
  howToPlay = false;  
  quit = false;  
  exet = false;  
  start = false;  
  teamGoing = false;  
  howToPlay = false;  
  topLeftActive = false;  
  topCenterActive = false;  
  topRightActive = false;  
  centerLeftActive = false;  
  centerCenterActive = false;  
  centerRightActive = false;  
  bottomLeftActive = false;  
  bottomCenterActive = false;  
  bottomRightActive = false;  
  topLeft1OverRide = false;  
  topCenter1OverRide = false;  
  topRight1OverRide = false;  
  centerLeft1OverRide = false;  
  centerCenter1OverRide = false;  
  centerRight1OverRide = false;  
  bottomLeft1OverRide = false;  
  bottomCenter1OverRide = false;  
  bottomRight1OverRide = false;  
  topLeft2OverRide = false;  
  topCenter2OverRide = false;  
  topRight2OverRide = false;  
  centerLeft2OverRide = false;  
  centerCenter2OverRide = false;  
  centerRight2OverRide = false;  
  bottomLeft2OverRide = false;  
  bottomCenter2OverRide = false;  
  bottomRight2OverRide = false;  
  topLeft1O = false;  
  topCenter1O = false;  
  topRight1O = false;  
  centerLeft1O = false;  
  centerCenter1O = false;  
  centerRight1O = false;  
  bottomLeft1O = false;  
  bottomCenter1O = false;  
  bottomRight1O = false;  
  topLeft2O = false;  
  topCenter2O = false;  
  topRight2O = false;  
  centerLeft2O = false;  
  centerCenter2O = false;  
  centerRight2O = false;  
  bottomLeft2O = false;  
  bottomCenter2O = false;  
  bottomRight2O = false;  
  topLeftMainO = false;  
  topCenterMainO = false;  
  topRightMainO = false;  
  centerLeftMainO = false;  
  centerCenterMainO = false;  
  centerRightMainO = false;  
  bottomLeftMainO = false;  
  bottomCenterMainO = false;  
  bottomRightMainO = false;  
  winTopLeftO = false;  
  topLeft1X = false;  
  topCenter1X = false;  
  topRight1X = false;  
  centerLeft1X = false;  
  centerCenter1X = false;  
  centerRight1X = false;  
  bottomLeft1X = false;  
  bottomCenter1X = false;  
  bottomRight1X = false;  
  topLeftMainX = false;  
  topCenterMainX = false;  
  topRightMainX = false;  
  centerLeftMainX = false;  
  centerCenterMainX = false;  
  centerRightMainX = false;  
  bottomLeftMainX = false;  
  bottomCenterMainX = false;  
  bottomRightMainX = false;  
}
